...
public interface TransactionEntityRepository extends JpaRepository<TransactionEntity, Long> {
  ...
  Page<TransactionEntity> findByUserIdAndActiveTrueOrderByCreatedAtDesc(Long userId, Pageable pageable);

  List<TransactionEntity> findByUserIdAndActiveTrueOrderByCreatedAtDesc(Long userId);

  List<TransactionEntity> findByUserIdOrderByCreatedAtDesc(Long userId);

  List<TransactionEntity> findByUserIdAndStatusAndActiveTrue(Long userId, TransactionStatus status);

  List<TransactionEntity> findByPhoneNumberAndActiveTrueOrderByCreatedAtDesc(String phoneNumber);

  Optional<TransactionEntity> findByExternalTransactionIdAndActiveTrue(String externalTransactionId);

  @Query("SELECT t FROM TransactionEntity t WHERE t.active = true AND " +
  ...
  List<TransactionEntity> findTop10ByUserIdAndActiveTrueOrderByCreatedAtDesc(Long userId);
}
